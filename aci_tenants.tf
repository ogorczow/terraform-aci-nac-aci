            ...
            for node in try(np.nodes, []) : [
              for static_route in try(node.static_routes, []) : [
                for next_hop in try(static_route.next_hops, []) : {
                  ...
                  tenant          = tenant.name
                  name            = try("${l3out.vrf}_${next_hop.ip}", null)
                  description     = try("Generated by l3out: ${l3out.name}, node: ${node.node_id}, nexthop: ${next_hop.ip}", "")
                  ...
                  track_members   = try(["${l3out.vrf}_${next_hop.ip}"], [])
